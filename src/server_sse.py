...
class _MobileSseHandler:
    """ASGI handler for /m/{chave}/sse - mobile SSE endpoint for Claude.ai app."""
...
# ============================================================================
# ASGI HANDLERS for message endpoints
# (Raw ASGI apps - bypasses Starlette Response wrapper that causes TypeError)
# ============================================================================
...
class _MobileMessagesHandler:
    """ASGI handler for /m/{chave}/messages - routes POST to mobile SSE transport."""
    ...
    async def __call__(self, scope, receive, send):
        ...
        if not chave:
            ...
            await response(scope, receive, send)
            return
        transport = _mobile_transports.get(chave)
        if not transport:
            response = Response(status_code=404)
            await response(scope, receive, send)
            return
...
async def lifespan(app):
    ...
    logger.info(f"TeamDesk MCP Server v3.1.1 (SSE + Mobile)")
...
routes = [
    ...
    Route("/m/{chave}/sse", _MobileSseHandler()),
    Route("/m/{chave}/messages", _MobileMessagesHandler()),
]
